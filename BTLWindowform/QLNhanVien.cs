...
namespace BTLWindowform
{
    public partial class QLNhanVien : Form
    {
        ...
        private bool KiemTraUserName()
        {
            ...
            {
                using (SqlDataAdapter mydata = new SqlDataAdapter("SELECT * FROM tblEmployee WHERE username = '" + txtTenDangNhap.Text +"'" , cnn))
                {
        ...
        private bool KiemTraCCCD()
        {
            ...
            using (SqlConnection cnn = new SqlConnection(constr))
            {
                using (SqlDataAdapter mydata = new SqlDataAdapter("SELECT * FROM tblEmployee WHERE CCCD = '" + txtCCCD.Text + "'", cnn))
                {
                    using (DataTable tbl = new DataTable())
                    {
                        mydata.Fill(tbl);
        ...
        private void dvgNV_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //}
            int i = dvgNV.CurrentRow.Index;
            txtMaNV.Text = dvgNV.Rows[i].Cells[0].Value.ToString();
            txtTenDangNhap.Text = dvgNV.Rows[i].Cells[1].Value.ToString();
            txtTenNV.Text = dvgNV.Rows[i].Cells[2].Value.ToString();
            if(dvgNV.Rows[i].Cells[3].Value.ToString() == "Nam")
            {
                radioButton1.Checked = true;
            }
            else
            ...
            if(dvgNV.Rows[i].Cells[3].Value.ToString() == "Nữ")
            {
                radioButton2.Checked = true;
            }
            else
            ...
            txtNgaySinh.Text = dvgNV.Rows[i].Cells[4].Value.ToString();
            txtDiaChi.Text = dvgNV.Rows[i].Cells[5].Value.ToString();
            txtSDT.Text = dvgNV.Rows[i].Cells[6].Value.ToString();
            txtCCCD.Text = dvgNV.Rows[i].Cells[7].Value.ToString();
