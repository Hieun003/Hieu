...
namespace BTLWindowform
{
    public partial class QLHoaDon : Form
    {
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Ngày lập hóa đơn không được lớn hơn ngày hiện tại");
                return;
            }
            string[] arrKhachhang = cbKhachhang.SelectedItem.ToString().Split('.');
            string[] arrNhanvien = cbNhanvien.SelectedItem.ToString().Split('.');
        ...
        private bool KiemTra()
        {
            ...
            {
                using (SqlDataAdapter mydata = new SqlDataAdapter("SELECT * FROM tblBill,tblEmployee WHERE tblBill.idEmployee = tblEmployee.idEmployee AND tblEmployee.idEmployee = " + textBox1.Text + "", cnn))
                {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            string Manhanvien = "SELECT * FROM v_HoaDon WHERE [Mã nhân viên] = " + textBox1.Text + "";
            if (KiemTra() == true)
            {
